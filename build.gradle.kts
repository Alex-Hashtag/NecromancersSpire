...
hytale {
    // Uncomment to add Assets.zip to external libraries
    // ⚠️ CAUTION: this file is very big and might make your IDE unresponsive!
    //
    // addAssetsDependency = true

    // Uncomment to develop against pre-release version of the game
    //
...
val syncAssets = tasks.register<Copy>("syncAssets") {
    ...
    // Take from temporary build folder (where game saved changes)
    from(layout.buildDirectory.dir("resources/main"))

    // Copy into actual project source (where code lives)
    into("src/main/resources")

    // IMPORTANT: Protect manifest template from being overwritten
    exclude("manifest.json")

    // If file exists, overwrite with new version from game
...
afterEvaluate {
    // Now Gradle will find it, because plugin has finished working
